...
export const ExecutionCertificateManager: React.FC<Props> = ({
  ...
  if (loading) return <div className="p-8 text-center text-slate-400">Cargando turnos facturables...</div>;
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 shadow-2xl mt-8">
      <div className="border-b border-slate-800 pb-6 mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 mt-2 text-sm max-w-lg">
            Selecciona los turnos completados para agruparlos de forma atómica en un certificado financiero. El ERP será notificado de inmediato.
          </p>
        </div>
        
        <div className="bg-slate-800 rounded-lg p-4 flex gap-6 text-right border border-slate-700">
          <div>
            <p className="text-xs text-slate-400 font-mono uppercase">Horas Sel.</p>
            <p className="text-lg text-white font-bold">{totalSelectedHours} hrs</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-mono uppercase">Ingreso Bruto</p>
            <p className="text-lg text-emerald-400 font-bold">${estimatedRevenue.toFixed(2)}</p>
          </div>
        </div>
      </div>
      ...
      {error && (
        <div className="bg-red-900/50 border border-red-500/50 text-red-200 p-4 rounded-lg mb-6 text-sm font-mono flex items-start gap-3">
          ...
          <div>
            <p className="font-bold uppercase mb-1 text-red-400">Violación Financiera</p>
            <p>{error}</p>
          </div>
        </div>
      )}
      ...
      {shifts.length === 0 ? (
        <div className="text-center py-12 border-2 border-dashed border-slate-800 rounded-lg bg-slate-900/50">
          <p className="text-slate-500 font-mono text-sm">No hay turnos completados pendientes de facturación en la flota.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800 bg-slate-900">
              {shifts.map((shift) => (
                <tr 
                  key={shift.id} 
                  className={`hover:bg-slate-800/80 transition-colors cursor-pointer ${selectedShiftIds.includes(shift.id) ? 'bg-blue-900/10' : ''}`}
                ...
                >
                  ...
                  <td className="p-4 text-slate-400 font-mono text-xs">
                    {shift.shift_start ? new Date(shift.shift_start).toLocaleString() : 'N/A'}
                  </td>
                  <td className="p-4 text-slate-400 font-mono text-xs">
                    {shift.shift_end ? new Date(shift.shift_end).toLocaleString() : 'N/A'}
                  </td>
      ...
      <div className="mt-8 pt-6 border-t border-slate-800 flex justify-end">
        <button
          onClick={onGenerate}
          disabled={selectedShiftIds.length === 0 || generating}
