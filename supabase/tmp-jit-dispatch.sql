...
BEGIN
    ...
    IF v_was_inside_pad AND NOT v_is_inside_pad AND (NEW.payload->>'category' = 'haul_truck') THEN
        ...
        IF v_closest_truck_id IS NOT NULL THEN
            ...
            -- Nota: Dependiendo de la versión de Supabase, pg_notify('pgrst') no enruta directamente a WebSockets
            -- pero dejaremos la implementación propuesta como concepto de arquitectura.
            PERFORM pg_notify(
                'pgrst',
                jsonb_build_object(
                    'table', 'assets',
